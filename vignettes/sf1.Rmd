...
reference system (CRS) is known; this may be an spheroid CRS such as WGS84, a projected, two-dimensional (Cartesian) CRS such as a UTM zone or Web Mercator, or a CRS
...
```{r}
st_write(nc, "nc.shp", update = FALSE)
```
...
* `update=FALSE` causes `st_write` to try to create the data
layer, overwriting if it exists; no errors are given if the data source is
